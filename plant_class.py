...
class Plant():
    ...
    def recepit(self):
        ...
        if add_cart != 0:
            ...
            #if it has one item in shopping cart do this.
            if add_cart == 1:
                print(f"{self.get_buy_plant_name()}       SAR {self.get_cost()}")
                print("\n--------------------------------")
                print(f"TOTAL AMOUNT             SAR {self.get_cost()}")
                print("--------------------------------")
            #if there are many items in shopping cart use "lambda" function to sum the total price.
            else:
